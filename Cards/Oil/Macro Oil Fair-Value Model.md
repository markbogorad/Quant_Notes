...
- Essentially a [[Multiple Linear Regression]] model that uses [[Cross-Asset Spread Arbitrage]] trading strategies as regressiors
	- Things like CAD/USD, CPI, and [[XOP]] can be a 3-factor macro model
- **Strategy:** forecast with your regression model and trade off that
...
## Practicalities
- Ex: model with CAD dollar, XOP, etc
- Do a rolling regression - will get a good fit
- If futs>estimate -> sell
- If futs$<$estimate -> buy
- Can do dynamic factor selection (rolling the best factors every few weeks)